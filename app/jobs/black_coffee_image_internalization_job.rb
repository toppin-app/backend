class BlackCoffeeImageInternalizationJob < ApplicationJob
  queue_as :default

  REENQUEUE_DELAY = 1.second

  def perform(batch_id, limit = BlackCoffeeImageInternalizationRunner::DEFAULT_LIMIT, worker_token = nil)
    batch = BlackCoffeeImageInternalizationBatch.find_by(id: batch_id)
    return unless runnable_batch?(batch, worker_token)

    mark_heartbeat!(batch, limit)
    BlackCoffeeImageInternalizationRunner.advance!(batch: batch, limit: limit)
    batch.reload
    return unless runnable_batch?(batch, worker_token)
    ...
    Rails.logger.error "Black Coffee image internalization job failed: #{e.class} - #{e.message}"
