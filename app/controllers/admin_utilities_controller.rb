...
class AdminUtilitiesController < ApplicationController
  ...
  def index
    ...
    # Estadísticas de is_connected
    @users_connected = User.where(is_connected: true).count
    redis = Redis.new(url: ENV["REDIS_URL"])
    @users_connected_redis = redis.smembers("online_users").size rescue 0
    @users_with_stale_connection = User.where(is_connected: true)
                                        .where("last_connection IS NULL OR last_connection < ?", DateTime.now - 1.minute)
                                        .count
