...
class BlackCoffeeVenueCombinationMatrix
  BLANK_STATE_KEY = '__sin_region__'.freeze
  ...
  def self.normalize_state_key(value)
    value.to_s.strip.presence || BLANK_STATE_KEY
  end

  def self.state_label(value)
    value.to_s == BLANK_STATE_KEY ? 'Sin region' : value.to_s
  end
  ...
  def build
    ...
    @scope.select(:id, :state, :category).find_each(batch_size: 1_000) do |venue|
      category = venue.category.to_s.strip.presence
      next if category.blank?

      state_key = self.class.normalize_state_key(venue.state)
      states << state_key
      categories << category
      ...
      combination_keys = ordered_states.product(ordered_categories)
      populated_combination_keys = venue_ids_by_combination.keys.sort_by { |state_key, category| [state_key.to_s, category.to_s] }
      empty_combination_keys = combination_keys.reject { |key| venue_ids_by_combination[key].any? }
